...
def Temperature_lookup(filter, lam, T_calc, fit, Thermo_calib, soot_coef):
    ...
    if exists:
        ...
        blackfly_red = pd.read_csv('Filters/CanonEOS_60D_response_red.csv', delimiter=';', header=None).values
        blackfly_grn = pd.read_csv('Filters/CanonEOS_60D_response_green.csv', delimiter=';', header=None).values
        blackfly_blu = pd.read_csv('Filters/CanonEOS_60D_response_blue.csv', delimiter=';', header=None).values
        #Chenge Delimiter for original tables
        red = griddata(blackfly_red[:,0], blackfly_red[:,1], (lam), method='cubic') #war vohrer cubic bei allen 3
        grn = griddata(blackfly_grn[:,0], blackfly_grn[:,1], (lam), method='cubic')
        blu = griddata(blackfly_blu[:,0], blackfly_blu[:,1], (lam), method='cubic')
        
        red_nofilter = red /1 #100
        grn_nofilter = grn /1 #100
        blu_nofilter = blu /1 #100
        ...
        if filter == 'None':
                ...
                lens_raw = pd.read_csv('Filters/Canon50mm1-8_ad.csv', delimiter=';', header=None).values
                camera_filter = griddata(FGB7_raw[:,0], FGB7_raw[:,1], (lam), method='cubic') / 100
                plt.figure()
                plt.plot(lam,camera_filter)
                camera_lens = griddata(lens_raw[:,0], lens_raw[:,1], (lam), method='cubic')/100
        ...
        for T in range(0, len(T_measure)):
            fun_R_thermo[T] = integrate.quad(lambda l: S_fun_filter(T_measure[T],l,red_filter,lambda_first,alpha_thermo), lam[0], lam[-1], maxp1=200, limit=maxlim)[0]
            fun_G_thermo[T] = integrate.quad(lambda l: S_fun_filter(T_measure[T],l,grn_filter,lambda_first,alpha_thermo), lam[0], lam[-1], maxp1=200, limit=maxlim)[0]
            fun_B_thermo[T] = integrate.quad(lambda l: S_fun_filter(T_measure[T],l,blu_filter,lambda_first,alpha_thermo), lam[0], lam[-1], maxp1=200, limit=maxlim)[0]
        ...
        if fit == True:
            ...
            for T in range(0, len(T_calc)):
                cor_R_thermo[T] = integrate.quad(lambda l: S_fun_filter(T_calc[T],l,red_filter_Mod,lambda_first,alpha_thermo), lam[0], lam[-1], maxp1=200, limit=maxlim)[0]
                cor_G_thermo[T] = integrate.quad(lambda l: S_fun_filter(T_calc[T],l,grn_filter_Mod,lambda_first,alpha_thermo), lam[0], lam[-1], maxp1=200, limit=maxlim)[0]
                cor_B_thermo[T] = integrate.quad(lambda l: S_fun_filter(T_calc[T],l,blu_filter_Mod,lambda_first,alpha_thermo), lam[0], lam[-1], maxp1=200, limit=maxlim)[0]
                
                fun_R_soot[T] = integrate.quad(lambda l: S_fun_filter(T_calc[T],l,red_filter_Mod,lambda_first,alpha_soot), lam[0], lam[-1], maxp1=200, limit=maxlim)[0]
                fun_G_soot[T] = integrate.quad(lambda l: S_fun_filter(T_calc[T],l,grn_filter_Mod,lambda_first,alpha_soot), lam[0], lam[-1], maxp1=200, limit=maxlim)[0]
                fun_B_soot[T] = integrate.quad(lambda l: S_fun_filter(T_calc[T],l,blu_filter_Mod,lambda_first,alpha_soot), lam[0], lam[-1], maxp1=200, limit=maxlim)[0]
            ...
            plt.figure(dpi=1200)
            plt.plot(T_measure,RG_measure, 'xr')
            plt.plot(T_measure,RB_measure, 'xg')
            plt.plot(T_measure,BG_measure, 'xb')
            plt.plot(T_calc,RG_thermo_cor, '--r')
            plt.plot(T_calc,RB_thermo_cor, '--g')
            plt.plot(T_calc,BG_thermo_cor, '--b')
            plt.plot(T_calc,RG_soot, '.r')
            plt.plot(T_calc,RB_soot, '.g')
            plt.plot(T_calc,BG_soot, '.b')
           
            
            plt.title('Comparision of calculated and corrected colour ratios')
            plt.show()
        ...
        plt.show()
        # plt.figure()
        # plt.plot(T_calc,RG_thermo, 'or')
        # plt.plot(T_calc,RB_thermo, 'og')
        # plt.plot(T_calc,BG_thermo, 'ob')
        # plt.plot(T_calc,RG_thermo_cor, '--r')
        # plt.plot(T_calc,RB_thermo_cor, '--g')
        # plt.plot(T_calc,BG_thermo_cor, '--b')
